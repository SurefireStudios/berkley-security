...
def process_single_city():
    ...
    index_path = r"c:\Users\Haz\Desktop\Berkley Security\index.html"
    filepath = os.path.join(base_dir, "newellton-la.html")
    ...
    # Apply county fix
    county = re.sub(r'(cameras|home security|security)\s+', '', county).strip()
    # just grab the last words before County/Parish to be safe, or just use the sub above.
    county_match2 = re.search(r'([A-Za-z]+(?:\s[A-Za-z]+)?\s(?:County|Parish))', county)
    if county_match2:
        county = county_match2.group(1).strip()
    ...
    new_html = re.sub(r'<img[^>]+src="\.\./src/img/cities/[^>]+>', replace_cover, new_html)
    new_html = new_html.replace('overflow-hidden shadow-2xl', 'overflow-hidden')
    new_html = new_html.replace('overflow-hidden shadow-xl', 'overflow-hidden')
    new_html = new_html.replace('overflow-hidden shadow-lg', 'overflow-hidden')
    new_html = new_html.replace('overflow-hidden shadow-md', 'overflow-hidden')
    new_html = new_html.replace('overflow-hidden shadow', 'overflow-hidden')
    
    # 3. Inject Nearby Areas
    ...
    if match:
        ...
        new_html = re.sub(r'<!-- NEARBY AREAS[\s\S]*?<section[^>]*>[\s\S]*?Serving[\s\S]*?(?:Areas|Communities)[\s\S]*?</section>', service_areas_block, new_html, count=1)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_html)
    
    print(f"Successfully processed and finalized {filename} (City: {city}, County: {county})")
